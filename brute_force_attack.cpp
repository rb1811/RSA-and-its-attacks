...
{	
	...
	//r1.Decrypt();
		...
		// The logic for Brute force attack starts here

		    mpz_t large;
			mpz_init(large);
			
			cout<< "\nBrute force Attack to guess P and Q"<< endl;

			mpz_root(large,r1.n_value,2);
			mpz_mod_ui(r1.temp,large,2);
			if(!mpz_cmp(r1.temp,r1.zero)) mpz_sub(large,large,r1.positive_1);
			
			
			
			while(1)
			{  
		    	mpz_mod(r1.temp, r1.n_value,large);
				if(!mpz_cmp(r1.temp,r1.zero)){
				 break;
				}
				else {
					mpz_sub(large,large,r1.positive_1);
				    mpz_sub(large,large,r1.positive_1);
				}
				};
			mpz_div(r1.temp,r1.n_value,large);
			cout << "\n values of P and Q : " << large <<"," <<r1.temp<<endl;
